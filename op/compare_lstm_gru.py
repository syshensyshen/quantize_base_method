...
class BiGRU(nn.Module):
    ...
    def forward(self, input, hn):
        # 初始化前向和后向 GRU 的隐藏状态
        forward_res_hn ,backward_res_hn = [], []
        # 前向传播
        for i in range(input.size(0)):
            # hn[0] = self.gru_forward(input[i, :, :], hn[0])
            hn[0] = gru_cell(input[i, :, :], 
                             hn[0], 
                             self.gru_forward.weight_ih, 
                             self.gru_forward.weight_hh, 
                             self.gru_forward.bias_ih, 
                             self.gru_forward.bias_hh)
            forward_res_hn.append(copy.deepcopy(hn[0]))

        # 反向传播
        for i in reversed(range(input.size(0))):
            hn[1] = gru_cell(input[i, :, :], 
                             hn[1],
                             self.gru_backward.weight_ih,
                             self.gru_backward.weight_hh,
                             self.gru_backward.bias_ih,
                             self.gru_backward.bias_hh)
            backward_res_hn.insert(0, copy.deepcopy(hn[1]))
...
class ORIBiGRU(nn.Module):
    ...
    def forward(self, input, hn=None):
        # GRU 层
...
def build_torch_lstm_gru():
    ...
    ori_bigru_model = ORIBiGRU(input_size, hidden_size, output_size)
    ...
    bigru_model.gru_forward.weight_hh = nn.Parameter(ori_bigru_model.gru.weight_hh_l0.detach())
    bigru_model.gru_forward.weight_ih = nn.Parameter(ori_bigru_model.gru.weight_ih_l0.detach())
    bigru_model.gru_forward.bias_hh = nn.Parameter(ori_bigru_model.gru.bias_hh_l0.detach())
    bigru_model.gru_forward.bias_ih = nn.Parameter(ori_bigru_model.gru.bias_ih_l0.detach())
    bigru_model.gru_backward.weight_hh = nn.Parameter(ori_bigru_model.gru.weight_hh_l0_reverse.detach())
    bigru_model.gru_backward.weight_ih = nn.Parameter(ori_bigru_model.gru.weight_ih_l0_reverse.detach())
    bigru_model.gru_backward.bias_hh = nn.Parameter(ori_bigru_model.gru.bias_hh_l0_reverse.detach())
    bigru_model.gru_backward.bias_ih = nn.Parameter(ori_bigru_model.gru.bias_ih_l0_reverse.detach())
    ...
    # 前向传播
    hn = torch.randn((2, batch_size, hidden_size))
    cn = torch.zeros((2, batch_size, hidden_size))    
    # ori_hn = torch.zeros((2, batch_size, hidden_size))
    # ori_cn = torch.zeros((2, batch_size, hidden_size))
    bilstm_output, bi_hn, bi_cn = bilstm_model(input_data.detach(), copy.deepcopy(hn), copy.deepcopy(cn))
    ori_bilstm_output, ori_hn, ori_cn = ori_bilstm_model(input_data.detach(), hn, cn)
    
    hx = torch.zeros((sequence_length, batch_size, hidden_size))
    # ori_hx = torch.zeros((2, batch_size, hidden_size))
    bigru_output = bigru_model(input_data.detach(), hx)
    ori_bigru_output = ori_bigru_model(input_data.detach())

    diff_lstm = bilstm_output.detach() - ori_bilstm_output.detach()
    diff_gru  = bigru_output.detach() - ori_bigru_output.detach()
    print(diff_lstm.max(), diff_lstm.min())
    print(diff_gru.max(), diff_gru.min())
