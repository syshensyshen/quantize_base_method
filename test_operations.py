...
def test_lstm(batch=1, time_step=9, input_size=64, hidden_size=32, bidirectional = True):
    ...
    hn = hn.detach().numpy()
    cn = cn.detach().numpy()
    W = torch.lstm.state_dict()['lstm_cell.fc1.weight'].unsqueeze(0)
    Wb1 = torch.lstm.state_dict()['lstm_cell.fc1.bias'].unsqueeze(0)
    R = torch.lstm.state_dict()['lstm_cell.fc2.weight'].unsqueeze(0)
    Wb2 = torch.lstm.state_dict()['lstm_cell.fc2.bias'].unsqueeze(0)
